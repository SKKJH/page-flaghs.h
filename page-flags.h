...
#define SETPAGEFLAG_REFERENCED(lname, policy)                          	\
static __always_inline void SetPageReferenced(struct page *page)       	\
{                                                                       \
    set_bit(PG_##lname, &policy(page, 1)->flags);                       \
    if (async_mod_glob_ctrl.initialized) {                              \
        async_mod_glob_ctrl.reference_master_page(page);                \
    }                                                                   \
}

#define SETPAGEFLAG_ACTIVE(lname, policy)                              	\
...
#define __SETPAGEFLAG_REFERENCED(lname, policy)                         \
...
#define __SETPAGEFLAG_ACTIVE(lname, policy)                             \
...
#define TESTSETFLAG_REFERENCED(lname, policy)                          	\
static __always_inline int TestSetPageReferenced(struct page *page)    	\
{                                                                       \
    int ret = test_and_set_bit(PG_##lname, &policy(page, 1)->flags);    \
    if (async_mod_glob_ctrl.initialized && !ret) {                      \
        async_mod_glob_ctrl.reference_master_page(page);                \
    }                                                                   \
    return ret;                                                         \
}

#define TESTSETFLAG_ACTIVE(lname, policy)                              	\
static __always_inline int TestSetPageActive(struct page *page)        	\
{                                                                       \
    int ret = test_and_set_bit(PG_##lname, &policy(page, 1)->flags);    \
    if (async_mod_glob_ctrl.initialized && !ret) {                      \
        async_mod_glob_ctrl.reference_master_page(page);                \
